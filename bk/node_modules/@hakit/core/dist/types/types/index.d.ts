import { HassEntity, HassServiceTarget, Context } from 'home-assistant-js-websocket';
import { DefaultServices } from './supported-services';
import { DefinedPropertiesByDomain } from './entitiesByDomain';
import { TimelineState, EntityHistoryState } from '../hooks/useHistory/history';
...
export type SupportedServices<T extends ServiceFunctionTypes = "target"> = [keyof CustomSupportedServices<T>] extends ["UNDETERMINED"] ? DefaultServices<T> : CustomSupportedServices<T>;
...
export type EntityName = ([keyof CustomEntityNameContainer] extends [never] ? DefaultEntityName : CustomEntityNameContainer["names"]) | "unknown";
...
export type HassEntityHelper<T extends AllDomains> = CamelToSnake<T> extends keyof DefinedPropertiesByDomain ? DefinedPropertiesByDomain[CamelToSnake<T>] : HassEntity;
export interface ServiceResponse<Response = object> {
    context: Context;
    response: Response;
}
export type HassEntityWithService<T extends AllDomains> = HassEntityCustom & HassEntityHelper<SnakeToCamel<T>> & {
    history: {
...
export type ServiceFunctionWithEntity<ResponseData extends object, Data = object> = {
    <CustomResponseData extends ResponseData = ResponseData>({ target, serviceData, returnResponse, }: {
    ...
    <CustomResponseData extends ResponseData = ResponseData>({ target, serviceData, }: {
        /** the entity target from home assistant, string, string[] or object */
        target: Target;
        /** the data to send to the service */
        serviceData?: Data;
...
export type ServiceFunction<ResponseData extends object, T extends ServiceFunctionTypes = "target", Data = object> = T extends "target" ? ServiceFunctionWithEntity<ResponseData, Data> : ServiceFunctionWithoutEntity<ResponseData, Data>;
...
export type SnakeToCamel<Key extends string> = Key extends `${infer FirstPart}_${infer FirstLetter}${infer LastPart}` ? `${FirstPart}${Uppercase<FirstLetter>}${SnakeToCamel<LastPart>}` : Key;
/** Will convert a string to snake case */
export type CamelToSnake<S extends string> = S extends `${infer T}${infer U}` ? T extends "_" ? `${T}${CamelToSnake<U>}` : T extends Uppercase<T> ? `_${Lowercase<T>}${CamelToSnake<U>}` : `${T}${CamelToSnake<U>}` : S;
/** Returns a union of all available services by domain in both snake and camel case */
export type DomainService<D extends SnakeOrCamelDomains> = NonSymbolNumberKeys<SupportedServices[SnakeToCamel<D>]> | CamelToSnake<NonSymbolNumberKeys<SupportedServices[SnakeToCamel<D>]>>;
/** returns the supported data to be used with the ServiceFunction */
export type ServiceData<D extends SnakeOrCamelDomains, S extends DomainService<D>> = S extends keyof SupportedServices[SnakeToCamel<D>] ? SupportedServices[SnakeToCamel<D>][S] extends ServiceFunction<object, "target", infer Params> ? Params : never : SnakeToCamel<S> extends keyof SupportedServices[SnakeToCamel<D>] ? SupportedServices[SnakeToCamel<D>][SnakeToCamel<S>] extends ServiceFunction<object, "target", infer Params> ? Params : never : never;
